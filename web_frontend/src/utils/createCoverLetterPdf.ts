...
async function loadGrayscaleLogo(pdfDocument: PDFDocument) {
  try {
    grayscaleLogoBytes ??= fetch(logoUrl).then(async (response) => {
      if (!response.ok) throw new Error("Unable to load the cover-letter logo.");
      return (await response.blob()).arrayBuffer();
    });
    return await pdfDocument.embedPng(await grayscaleLogoBytes);
  } catch {
